...
flags.DEFINE_string("algo", None, '["td3", "sac", "saca", "sace", \
                                    "bcd", "bcd-sacd", \
                                    "bc2sac", \
                                    "bcd2scd", "bc", "sacd", \
                                    "sacde", "sacda", "bc-sac"]')
...
flags.DEFINE_float("advisor_alpha", None, "for advisor [sacda, saca]")
flags.DEFINE_float("w_bc", None, "for [bcd-sacd, bc-sac]")
flags.DEFINE_float("bc_perc", None, "for [bcd2sacd, bc2sac]")
...
if FLAGS.advisor_alpha is not None:
    assert v["policy"]["algo"] in ['saca', 'sacda']
    v["policy"]["alpha"] = FLAGS.advisor_alpha
if FLAGS.w_bc is not None:
    assert v["policy"]["algo"] in ['bcd-sacd', 'bc-sac']
    v["policy"]["w_bc"] = FLAGS.w_bc
if FLAGS.bc_perc is not None:
    assert v["policy"]["algo"] in ['bcd2sacd', 'bc2sac']
    v["policy"]["bc_perc"] = FLAGS.bc_perc

if FLAGS.seed is not None:
    v["seed"] = FLAGS.seed

if socket.gethostname() in ['theseus']:
...
assert algo in ["td3", "sac", "saca", "sace", "bcd",
                "bc-sac", "bcd-sacd", "bcd2sacd",
                "bc2sac", "sacdf",
                "bc", "sacd", "sacde", "sacda"]
if arch == "mlp":
    if oracle:
...
if algo in ["sac", "sacd", "sace", "saca", "sacde", "sacda"]:
