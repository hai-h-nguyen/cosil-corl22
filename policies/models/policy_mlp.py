...
class ModelFreeOffPolicy_MLP(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if isinstance(obs_dim, tuple) and len(obs_dim) == 3:
            ...
            obs_dim = self.actor_state_encoder.get_output_size()
    ...
    def act(
        ...
        if self.actor_state_encoder is not None:
            obs = self.actor_state_encoder(obs)
    ...
    def update(self, batch):
        ...
        if self.critic_state_encoder is not None:
            critic1_obs = self.critic_state_encoder(obs)
            critic2_obs = self.critic_state_encoder(obs)
            critic1_next_obs = self.critic_state_encoder(next_obs)
            critic2_next_obs = self.critic_state_encoder(next_obs)
        else:
            critic1_obs = obs
            critic1_obs = obs
            critic1_next_obs = next_obs
            critic2_next_obs = next_obs
        ...
        with torch.no_grad():
            ...
            if self.algo in [self.TD3_name, self.SAC_name]:
                next_q1 = self.qf1_target(critic1_next_obs, next_action)  # (B, 1)
                next_q2 = self.qf2_target(critic2_next_obs, next_action)
            else:
                next_q1 = self.qf1_target(critic1_next_obs)  # (B, A)
                next_q2 = self.qf2_target(critic2_next_obs)
        ...
        if self.algo in [self.TD3_name, self.SAC_name]:
            q1_pred = self.qf1(critic1_obs, action)
            q2_pred = self.qf2(critic2_obs, action)
        else:
            action = action.long()  # (B, 1)
            q1_pred = self.qf1(critic1_obs)
            q2_pred = self.qf2(critic2_obs)
        ...
        if self.algo in [self.TD3_name, self.SAC_name]:
            q1 = self.qf1(critic1_obs, new_action)
            q2 = self.qf2(critic2_obs, new_action)
        else:
            q1 = self.qf1(critic1_obs)
            q2 = self.qf2(critic2_obs)
